    ...
    switch (true) {
      ...
      case receivedLine.startsWith('R'): // RDS HEX
        ...
        if (groupType === 0 && (errors & 0x30) === 0) {
          // Bits 3-2 select the DI flag; bit 1 carries its value
          const diIndex = (blockB >> 2) & 0x3;
          const diVal = (blockB >> 1) & 0x1;
          if (diIndex <= 2) {
            dataToSend.di = (dataToSend.di & ~(1 << diIndex)) | (diVal << diIndex);
          }
          // Bit 0 indicates whether PTY codes are dynamically switched
          const ptyi = blockB & 0x1;
          dataToSend.di = (dataToSend.di & ~(1 << 3)) | (ptyi << 3);
        }
